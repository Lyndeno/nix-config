...
pkgs.runCommand "matugen-base16-check" {
  ...
  if ! ${pkgs.diffutils}/bin/diff -u ${../base16-matugen.yaml} ${perSystem.self.matugen-base16}; then
    echo "base16-matugen.yaml is out of date, run:"
    echo '  install -m 644 "$(nix build --no-link --print-out-paths .#matugen-base16)" base16-matugen.yaml'
    exit 1
  fi
  touch $out
''
